...
R0~R8은 **셸·매핑·커밋** 트랙이다. **타겟 추출**은 `#164`~`#174`가 정본이며 최우선이다 (`37_ENPivot_CanonicalLayer_174.md`).  
R5 배포 HUMAN은 막지 않는다. R6 코드에 추출 결함 우회를 넣지 말 것. `#174` 0단계는 코드 금지.
...
### 선행
R5 · S6 결론  
금지: `#170`/`#172`/`#174` 추출 결함을 본 차수 커밋 로직으로 우회 구현하지 말 것.
