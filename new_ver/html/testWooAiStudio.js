 ...
 * - loadSqlList / loadSqlItem / deleteSqlItem — WF별 SQL 이력(+삭제 확인)
 ...
 * - embed WebView: URLSearchParams·forEach·filter 사용 금지 (#131 흰화면)
 */
(function () {
  "use strict";
  ...
  function post(endpoint, payload) {
    // csrf/pageHost: urlViewer 가 X-Requested-With·Origin·Referer 를 서버에 안 넘길 때 CSRF 폴백
    var p = payload || {};
  ...
  function renderSlotResults(slots, partialPreview) {
    ...
    for (var si = 0; si < list.length; si++) {
      ...
      if (s.evidence) {
        ...
        det.innerHTML = "<summary>\uC2E4\uC81C \uC870\uD68C \uACB0\uACFC (\uD655\uC778 \uADFC\uAC70)</summary><pre class='code'>" +
  ...
  function setBindPick(aiSqlId) {
    if (!WORKFLOW_NAME || aiSqlId == null || aiSqlId === "" || aiSqlId === 0) {
      return Promise.resolve();
    }
    return post("testWooAiValidate.jssp", {
      action: "setBindPick",
      workflow_name: WORKFLOW_NAME,
      ai_sql_id: aiSqlId
  ...
  function _showSqlSide(on) {
    try {
      if (on) document.body.className =
        (document.body.className || "").replace(/\bhas-sql-side\b/g, "") + " has-sql-side";
      else document.body.className =
        (document.body.className || "").replace(/\bhas-sql-side\b/g, "");
      document.body.className = document.body.className.replace(/^\s+|\s+$/g, "").replace(/\s+/g, " ");
    } catch (eCls) {}
    var side = $("sideSql");
    if (!side) return;
    ...
    post("testWooAiValidate.jssp", {
      ...
      for (var i = 0; i < items.length; i++) {
        (function (it) {
          ...
          el.appendChild(meta);
          el.appendChild(del);
          el.onclick = function () { loadSqlItem(it.id); };
          box.appendChild(el);
        })(items[i]);
      }
    }).catch(function (e) {
      ...
      try {
        ...
        if (h) {
          ...
          h.textContent = "SQL list error: " + String(e);
        }
      } catch (e2) {}
    });
  }

  function deleteSqlItem(id, title) {
    var msg =
      "ai_sql_id=" + id +
      (title ? (" (" + title + ")") : "") +
      "\n\uC815\uB9D0 \uC774 \uC774\uB825\uC744 \uC0AD\uC81C\uD560\uAE4C\uC694?";
