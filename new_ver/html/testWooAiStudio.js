 ...
 * - renderChips / _chipDisplay — 칩·해석 한 줄은 params 값 우선(U4)
 *
 * [Dependencies]
 * =========
 ...
 * - Mirror of /woo/testWooAiStudioJs.jssp?v=180 · 스키마 반영(injected_at)이면 [WKF로 이동] 활성
...
(function () {
  ...
  function _syncRegButton() {
    ...
    } else if (ctx.wfHasInjected && _activeWorkflowName()) {
    ...
    post("testWooAiValidate.jssp", {
      ...
      if (opts.selectCurrent && ctx.wfHasInjected) {
        ...
        if (hSel) {
          ...
          hSel.textContent = _hasCompose()
            ? "\uC774 WKF\uC5D0\uB294 \uC774\uBBF8 \uBC18\uC601\uB41C SQL\uC774 \uC788\uC2B5\uB2C8\uB2E4. [WKF\uC5D0 \uB36E\uC5B4\uC4F0\uAE30]\uB85C \uC9C0\uAE08 \uC870\uAC74\uC744 \uB123\uC2B5\uB2C8\uB2E4."
    ...
    try {
      ...
      try {
        var dm0 = (typeof document.documentMode !== "undefined") ? String(document.documentMode) : "n/a(non-IE)";
          var okMsg = "js ok | documentMode=" + dm0 + " | embed=" + (EMBED ? "1" : "0") + " | match=" + (MATCH_ENABLED ? "1" : "0") + " | phase=" + ctx.phase + " | v=180";
